...
import tot_en
import re
from run_lexicon_en import create_index, METRICS
import ir_datasets
...
if __name__ == "__main__":
    ...
    if dataset.has_qrels(): 
        ...
        # 1. Construct a mapping from query ID to domain
        qid_to_domain = {}
        for q in dataset.queries_iter():
            # Obtain the domain attribute; default to 'unknown' if not present
            qid_to_domain[q.query_id] = getattr(q, 'domain', 'unknown')

        # 2. Aggregate results by domain
        domain_metric_values = defaultdict(lambda: defaultdict(list))
        
        # Traverse evaluation results and group by domain
        ...
        # 3. Calculate mean and std for each metric within each domain and log the results
        ...
        log.info(f"=== Overall Results ({args.split}) ===") # 建议加上这一行区分
        for metric, (mean, std) in eval_res_agg.items(): 
            log.info(f"{metric:<12}: {mean:.4f} ({std:0.4f})") 
    else: 
        log.info("dataset has no qrels. no eval performed!")
