...
import tot_zh
import re
from run_lexicon_zh import create_index, METRICS
import ir_datasets
...
def create_title_index(
    ...
    for raw_doc in tqdm(dataset.docs_iter(), desc="gathering aliases"):
        aliases[raw_doc.doc_id] = {cc.convert(raw_doc.title)}

        if gather_wikidata_aliases:
            went = wikicache.get(raw_doc.doc_id)
            if went:
                al = went["aliases"]
                if "en" in al:
                    for a in al["en"]:
                        aliases[raw_doc.doc_id].add(cc.convert(a["value"]))

        # remove braces and add to aliases
        no_br = set()
        for _ in aliases[raw_doc.doc_id]:
            converted_text = cc.convert(_)
            no_br.add(remove_braces(converted_text))
        aliases[raw_doc.doc_id].update(no_br)

    with open(os.path.join(docs_folder, "docs.jsonl"), "w", encoding='utf-8') as writer: # 建议显式指定 encoding
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metrics",
        required=False,
        default="recall_10,recall_100,recall_1000,ndcg_cut_10,ndcg_cut_100,ndcg_cut_1000,recip_rank",
    ...
    AL_PATH = f"./aliases_{args.index_name}.json"
    if not os.path.exists(index):
        ...
        serializable_aliases = {k: list(v) for (k, v) in aliases.items()}
        with open(AL_PATH, "w", encoding='utf-8') as f:
            json.dump(serializable_aliases, f, ensure_ascii=False)
    else:
        with open(AL_PATH, "r", encoding='utf-8') as f:
            aliases = json.load(f)
        # aliases = utils.read_json(AL_PATH) # 原来的 utils 可能没有指定 encoding
    ...
    for query in queries:
        converted_queries = [cc.convert(t) for t in query["gpt_queries"]]
        titles.extend(converted_queries)
        query["gpt_queries"] = converted_queries
    ...
    if dataset.has_qrels(): 
        ...
        log.info(f"=== Domain-specific Results ({args.split}) ===")
        ...
        log.info(f"=== Overall Results ({args.split}) ===") 
