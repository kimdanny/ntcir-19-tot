...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metrics",
        required=False,
        default=run_lexicon_en.METRICS,
    ...
    tot_en.register(args.data_path)
    ...
    if args.freeze_base_model:
        ...
        if "bge-m3" in args.model_or_checkpoint.lower():
            if args.embed_size > 10000: 
             ...
             log.info(f"Detected DPR-XM model (Setting language to en_XX)...")
    ...
    for split in {"train", "dev"}:
        irds_splits[split] = ir_datasets.load(f"en-corpus:{split}")
    ...
    try:
        ...
        irds_splits["test"] = ir_datasets.load(f"en-corpus:test")
        log.info("success!")
    except KeyError:
