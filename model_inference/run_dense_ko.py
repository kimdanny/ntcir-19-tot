...
from sentence_transformers import SentenceTransformer, losses, models, SparseEncoder
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metrics",
        required=False,
        default=run_lexicon_ko.METRICS,
    ...
    tot_ko.register(args.data_path)
    ...
    if args.freeze_base_model:
        ...
        if "bge-m3" in args.model_or_checkpoint.lower():
            if args.embed_size > 10000: 
             ...
             log.info(f"Detected DPR-XM model (Setting language to ko_KR)...")
            ...
            log.info(f"Loading Granite model (Standard)...")
            model = SentenceTransformer(args.model_or_checkpoint, device=args.device)
        elif "Gameselo" in args.model_or_checkpoint:
            log.info(f"Loading Gameselo STS model (Standard)...")
            model = SentenceTransformer(args.model_or_checkpoint, device=args.device)
        elif "bge" in args.model_or_checkpoint:
            log.info(f"Detected BGE model...")
            model = BGEWrapper(args.model_or_checkpoint, device=args.device)
        elif "gte-small" in args.model_or_checkpoint:
            log.info(f"Loading GTE-Small model: {args.model_or_checkpoint}")
            model = SentenceTransformer(args.model_or_checkpoint, device=args.device)
        else:
    ...
    for split in {"train", "dev"}:
        irds_splits[split] = ir_datasets.load(f"ko-corpus:{split}")

    if hasattr(model, "set_query_mode"):
        model.set_query_mode(False) # E5 Docs: No instruction
    ...
    try:
        ...
        irds_splits["test"] = ir_datasets.load(f"ko-corpus:test")
        log.info("success!")
    except KeyError:
    ...
    for split, dataset in irds_splits.items():
        ...
        if hasattr(model, "set_query_mode"):
            model.set_query_mode(True) # E5 Queries: With instruction
