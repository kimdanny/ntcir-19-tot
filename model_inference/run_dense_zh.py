...
class StandardWrapper:
    def __init__(self, model_name, device="cuda", **kwargs):
        log.info(f"Loading Standard Embedder (with OpenCC): {model_name}...")
...
class OpenCCSentenceTransformer(SentenceTransformer):
    def encode(self, sentences, **kwargs):
        # Convert traditional to simplified Chinese
        if isinstance(sentences, str):
            sentences = [sentences]
        if isinstance(sentences, list):
            sentences = [cc.convert(s) for s in sentences]
...
class MultilingualDPRWrapper:
    ...
    def __init__(self, model_name, device="cuda"):
        self.device = device
        self.is_query_mode = True # 默认为 Query 模式
    ...
    def encode(
        ...
        **kwargs
        ...
        # [ZH Specific] Convert traditional to simplified Chinese
        texts = [cc.convert(t) for t in texts]
...
class BGEM3Wrapper:
    """BGE-M3 Dense Mode (1024 dim) - With OpenCC"""
    def __init__(self, model_name, device="cuda"):
...
class BGEM3SparseWrapper:
    ...
    """
    def __init__(self, model_name, device="cuda"):
        print(f"Loading BGE-M3 (Sparse Mode - Robust): {model_name}...")
        try:
            from FlagEmbedding import BGEM3FlagModel
        except ImportError:
    ...
    def encode(self, texts, batch_size=32, convert_to_numpy=True, show_progress_bar=True, **kwargs):
        ...
        # Tokenize (max_length=512 for speed)
...
class SparseIndex:
    """Scipy Sparse Matrix Index for High-Dim Vectors"""
...
def encode_dataset_sparse(model, dataset, encode_batch_size):
    """Encoding pipeline for Sparse Models (avoids OOM)"""
    docs = []
    docid_to_idx = {}
    idx_to_docid = {}
...
class QZhouWrapper:
    def __init__(self, model_name, device="cuda"):
        super().__init__()
        self.device = device
        print(f"Loading QZhou Embedder: {model_name}...")
        
        # 1. Load via SentenceTransformer with trust_remote_code=True
        # QZhou requires custom code execution for its specific architecture
        ...
        # Default task instruction from Model Card
        self.task_instruction = "Given a web search query, retrieve relevant passages that answer the query"
        
        # Internal state to distinguish Query vs Doc encoding
        self.is_query_mode = False 

    def get_prompteol_input(self, text: str) -> str:
        """Wraps text in the PromptEOL template required by QZhou."""
        return f"This sentence: <|im_start|>“{text}” means in one word: “"

    def get_detailed_instruct(self, task_description: str, query: str) -> str:
        """Adds task instruction to queries."""
        return f'Instruct: {task_description}\nQuery:{query}'

    def set_query_mode(self, is_query: bool):
        """Toggle between Query encoding (with instruction) and Doc encoding."""
        self.is_query_mode = is_query
    ...
    def encode(
        ...
        **kwargs
        ...
        # --- Apply QZhou Specific Formatting ---
        formatted_texts = []
        for t in texts:
            if self.is_query_mode:
                # Logic for QUERIES: Instruct + PromptEOL
                instruct_text = self.get_detailed_instruct(self.task_instruction, t)
                final_text = self.get_prompteol_input(instruct_text)
            else:
                # Logic for DOCUMENTS: Just PromptEOL
                final_text = self.get_prompteol_input(t)
            formatted_texts.append(final_text)
        # ---------------------------------------

        # Delegate to the internal SentenceTransformer model
        # It handles batching, device, and normalization internally
        return self.model.encode(
            formatted_texts,
            batch_size=batch_size,
            convert_to_numpy=convert_to_numpy,
            show_progress_bar=show_progress_bar,
            normalize_embeddings=True # QZhou usually expects normalized embeddings for cosine sim
        )
...
class Qwen3Wrapper:
    def __init__(self, model_name, device="cuda"):
        self.device = device
        print(f"Loading Qwen3 Embedder: {model_name}...")
        
        # 1. Load via SentenceTransformer
    ...
    def encode(
        ...
        **kwargs
        ...
        # --- Apply Qwen3 Specific Formatting ---
        formatted_texts = []
        for t in texts:
            if self.is_query_mode:
                # Queries get the instruction wrapper
                final_text = self.get_detailed_instruct(self.task_instruction, t)
            else:
        ...
        # Delegate to internal model
        return self.model.encode(
            formatted_texts,
            batch_size=batch_size,
            convert_to_numpy=convert_to_numpy,
            show_progress_bar=show_progress_bar,
            normalize_embeddings=True # Qwen3 usually requires normalization
        )

# --- Yinka Wrapper (MRL Support) ---
class YinkaWrapper:
    def __init__(self, model_name, device="cuda", target_dim=None):
        self.device = device
        print(f"Loading Yinka Embedder: {model_name} with target dim: {target_dim}...")
    ...
    def encode(
        ...
        **kwargs
        ...
        # 1. Encode WITHOUT normalization first (Critical for MRL!)
        embeddings = self.model.encode(
            texts,
            batch_size=batch_size,
            convert_to_tensor=True, # Keep as tensor for efficient slicing on GPU
            show_progress_bar=show_progress_bar,
            normalize_embeddings=False 
        )

        # 2. MRL Slicing: Cut to the target dimension (e.g., 768 or 1792)
        if self.target_dim:
            embeddings = embeddings[:, :self.target_dim]

        # 3. Normalize AFTER slicing (Critical!)
        embeddings = torch.nn.functional.normalize(embeddings, p=2, dim=1)

        # 4. Convert to numpy if requested
        if convert_to_numpy:
            embeddings = embeddings.cpu().numpy()
            
        return embeddings

# --- GME Wrapper (Qwen2-VL) ---
class GMEWrapper:
    def __init__(self, model_name, device="cuda"):
        self.device = device
        print(f"Loading GME Embedder: {model_name}...")
    ...
    def encode(self, texts, batch_size=32, convert_to_numpy=True, show_progress_bar=True, device=None, **kwargs):
        # Ensure input is a list
        if isinstance(texts, str):
            texts = [texts]
        ...
        for t in texts:
            input_item = {"text": t}
            if self.is_query_mode:
                input_item["prompt"] = self.task_instruction
            formatted_inputs.append(input_item)
...
class BGEWrapper:
    def __init__(self, model_name, device="cuda"):
        ...
        # BGE is a standard SentenceTransformer model
        self.model = SentenceTransformer(model_name, device=device)
        
        # The specific instruction required by BGE-v1.5 for retrieval queries
    ...
    def encode(
        ...
        **kwargs
        ...
        # --- Apply BGE Specific Instruction ---
        formatted_texts = []
        if self.is_query_mode:
            # Queries need instruction prepended
            formatted_texts = [self.query_instruction + t for t in texts]
        else:
            # Documents do not need instruction
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metrics",
        required=False,
        default=run_lexicon_zh.METRICS,
    ...
    if args.freeze_base_model:
        if args.model_or_checkpoint == "facebook/dpr-question_encoder-single-nq-base":
            ...
            model = OpenCCSentenceTransformer(
        ...
        if "bge-m3" in args.model_or_checkpoint.lower():
            if args.embed_size > 10000: 
             ...
             log.info(f"Detected DPR-XM model (Setting language to zh_CN)...")
        ...
        elif "Ops-MoA-Yuan" in args.model_or_checkpoint:
            log.info(f"Loading Ops-MoA-Yuan model: {args.model_or_checkpoint}")
            model = StandardWrapper( # Changed from SentenceTransformer
                args.model_or_checkpoint, 
                device=args.device, 
                trust_remote_code=True
            )
        elif "ritrieve" in args.model_or_checkpoint:
            log.info(f"Loading ritrieve_zh model: {args.model_or_checkpoint}")
            model = StandardWrapper(args.model_or_checkpoint, device=args.device) # Changed
        elif "gte-large-zh" in args.model_or_checkpoint:
            log.info(f"Loading gte-large-zh model: {args.model_or_checkpoint}")
            model = StandardWrapper(args.model_or_checkpoint, device=args.device) # Changed
        elif "QZhou" in args.model_or_checkpoint:
            log.info(f"Detected QZhou model, using QZhouWrapper: {args.model_or_checkpoint}")
            model = QZhouWrapper(args.model_or_checkpoint, device=args.device)
        elif "Qwen3" in args.model_or_checkpoint:
            log.info(f"Detected Qwen3 model, using Qwen3Wrapper...")
            model = Qwen3Wrapper(args.model_or_checkpoint, device=args.device)
        elif "Yinka" in args.model_or_checkpoint:
            log.info(f"Detected Yinka model (MRL), target dim: {args.embed_size}...")
            model = YinkaWrapper(
                args.model_or_checkpoint, 
                device=args.device, 
                target_dim=args.embed_size 
            )
        elif "gme" in args.model_or_checkpoint:
            log.info(f"Detected GME model, using GMEWrapper...")
            model = GMEWrapper(args.model_or_checkpoint, device=args.device)
        elif "bge" in args.model_or_checkpoint:
            log.info(f"Detected BGE model, using BGEWrapper...")
            model = BGEWrapper(args.model_or_checkpoint, device=args.device)
        else:
            log.info(f"Loading standard SentenceTransformer: {args.model_or_checkpoint}")
            model = StandardWrapper(args.model_or_checkpoint, device=args.device) # Changed
    ...
    for split in {"train", "dev"}:
        irds_splits[split] = ir_datasets.load(f"zh-corpus:{split}")
    ...
    # )
    # [MODIFIED] Branching for Sparse vs Dense
    ...
    try:
        ...
        irds_splits["test"] = ir_datasets.load(f"zh-corpus:test")
        log.info("success!")
    except KeyError:
