...
OUTPUT_JA_ONLY = os.path.join(OUTPUT_DIR, "wikipedia_ja_only.jsonl")
OUTPUT_BILINGUAL = os.path.join(OUTPUT_DIR, "wikipedia_bilingual_ja_en.jsonl")
...
# ==============================================================================
def get_english_links_batch(japanese_titles: List[str]) -> Dict[str, Optional[str]]:
    """
    Finds English titles for a given BATCH of Japanese titles in a single API call.
    Returns a dictionary mapping each Japanese title to its English counterpart.
    """
    titles_param = "|".join(japanese_titles)
    encoded_titles = urllib.parse.quote(titles_param)
    
    # Use the Japanese Wikipedia API endpoint with multiple titles
    url = f"https://ja.wikipedia.org/w/api.php?action=query&prop=langlinks&titles={encoded_titles}&lllang=en&format=json&redirects"

    # Initialize a result map with None for all requested titles
    results_map = {title: None for title in japanese_titles}
    ...
    try:
        ...
        for page in pages.values():
            japanese_title_from_api = page.get('title')
            if not japanese_title_from_api:
                continue
            ...
            if japanese_title_from_api in results_map:
                results_map[japanese_title_from_api] = english_link

        # Map results from redirected/normalized titles back to the original titles
        for original_title in japanese_titles:
        ...
        print(f"    -> Batch API Error for titles starting with '{japanese_titles[0]}': {e}")
        return results_map # Return the map with Nones on error

# --- Load the processed titles ---
def load_processed_titles(ja_only_path: str, bilingual_path: str) -> Set[str]:
    """Reads existing output files and builds a set of all processed Japanese article titles."""
    processed_titles = set()
    for filepath in [ja_only_path, bilingual_path]:
        if not os.path.exists(filepath):
            continue
        print(f"Loading processed Japanese titles from {filepath}...")
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    data = json.loads(line)
                    if 'title_ja' in data:
                        processed_titles.add(data['title_ja'])
...
def main():
    ...
    # Load already processed Japanese titles to avoid duplication
    processed_ja_titles = load_processed_titles(OUTPUT_JA_ONLY, OUTPUT_BILINGUAL)
    if processed_ja_titles:
        print(f"Resuming job. Found {len(processed_ja_titles)} already processed Japanese titles. They will be skipped.")
    ...
    print(f"Loading Japanese Wikipedia dataset ({DATASET_CONFIG_JA})...")
    ds_ja_full = load_dataset("/dataset/", DATASET_CONFIG_JA, split='train')
    ...
    if NUM_SAMPLES_TO_TEST and NUM_SAMPLES_TO_TEST > 0:
        ...
        ds_ja = ds_ja_full.take(NUM_SAMPLES_TO_TEST)
    else:
    ...
    with open(OUTPUT_JA_ONLY, "a", encoding="utf-8") as f_ja_only, \
         open(OUTPUT_BILINGUAL, "a", encoding="utf-8") as f_bilingual:
        print("Starting to process the Japanese dataset and classify articles...")
        
        # Manually manage tqdm for accurate progress tracking with batching
        progress_bar = tqdm(total=len(ds_ja_full), desc="Processing Japanese articles")
        progress_bar.update(len(processed_ja_titles)) # Set initial progress

        for item_ja in ds_ja_full:
            japanese_title = item_ja['title']
            if japanese_title in processed_ja_titles:     # Check if already processed
                continue
            
            # Add item to the current batch
            batch.append(item_ja)
            ...
            if len(batch) >= BATCH_SIZE:
                ...
                for item_in_batch in batch:
                    ...
                    if result:
                        ...
                        bilingual_record = {
                            "id_ja": item_in_batch['id'], "url_ja": item_in_batch['url'],
                            "title_ja": title_in_batch, "text_ja": item_in_batch['text'],
                            "id_en": item_en['id'],
                        }
                        ...
                        f_ja_only.write(json.dumps(ja_only_record, ensure_ascii=False) + "\n")
        ...
        if batch:
            ...
            for item_in_batch in batch:
                ...
                if result:
                    item_en = json.loads(result[0])
                    bilingual_record = {"id_ja": item_in_batch['id'], "url_ja": item_in_batch['url'], "title_ja": title_in_batch, "text_ja": item_in_batch['text'], "id_en": item_en['id']}
                    f_bilingual.write(json.dumps(bilingual_record, ensure_ascii=False) + "\n")
                else:
                    ja_only_record = {"id_ja": item_in_batch['id'], "url_ja": item_in_batch['url'], "title_ja": title_in_batch, "text_ja": item_in_batch['text']}
                    f_ja_only.write(json.dumps(ja_only_record, ensure_ascii=False) + "\n")
