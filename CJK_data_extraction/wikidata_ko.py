...
OUTPUT_KO_ONLY = os.path.join(OUTPUT_DIR, "wikipedia_ko_only.jsonl")
OUTPUT_BILINGUAL = os.path.join(OUTPUT_DIR, "wikipedia_bilingual_ko_en.jsonl")
...
# ==============================================================================
def get_english_links_batch(korean_titles: List[str]) -> Dict[str, Optional[str]]:
    """
    Finds English titles for a given BATCH of Korean titles in a single API call.
    Returns a dictionary mapping each Korean title to its English counterpart.
    """
    titles_param = "|".join(korean_titles)
    encoded_titles = urllib.parse.quote(titles_param)
    
    # Use the Korean Wikipedia API endpoint with multiple titles
    url = f"https://ko.wikipedia.org/w/api.php?action=query&prop=langlinks&titles={encoded_titles}&lllang=en&format=json&redirects"

    # Initialize a result map with None for all requested titles
    results_map = {title: None for title in korean_titles}
    ...
    try:
        ...
        for page in pages.values():
            korean_title_from_api = page.get('title')
            if not korean_title_from_api:
                continue
            ...
            if korean_title_from_api in results_map:
                results_map[korean_title_from_api] = english_link

        # Map results from redirected/normalized titles back to the original titles
        for original_title in korean_titles:
        ...
        print(f"    -> Batch API Error for titles starting with '{korean_titles[0]}': {e}")
        return results_map # Return the map with Nones on error

# --- Load the processed titles ---
def load_processed_titles(ko_only_path: str, bilingual_path: str) -> Set[str]:
    """Reads existing output files and builds a set of all processed Korean article titles."""
    processed_titles = set()
    for filepath in [ko_only_path, bilingual_path]:
        if not os.path.exists(filepath):
            continue
        print(f"Loading processed Korean titles from {filepath}...")
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    data = json.loads(line)
                    if 'title_ko' in data:
                        processed_titles.add(data['title_ko'])
...
def main():
    ...
    # Load already processed Korean titles to avoid duplication
    processed_ko_titles = load_processed_titles(OUTPUT_KO_ONLY, OUTPUT_BILINGUAL)
    if processed_ko_titles:
        print(f"Resuming job. Found {len(processed_ko_titles)} already processed Korean titles. They will be skipped.")
    ...
    print(f"Loading Korean Wikipedia dataset ({DATASET_CONFIG_KO})...")
    ds_ko_full = load_dataset("/dataset/", DATASET_CONFIG_KO, split='train')
    ...
    if NUM_SAMPLES_TO_TEST and NUM_SAMPLES_TO_TEST > 0:
        ...
        ds_ko = ds_ko_full.take(NUM_SAMPLES_TO_TEST)
    else:
    ...
    with open(OUTPUT_KO_ONLY, "a", encoding="utf-8") as f_ko_only, \
         open(OUTPUT_BILINGUAL, "a", encoding="utf-8") as f_bilingual:
        print("Starting to process the Korean dataset and classify articles...")
        
        # Manually manage tqdm for accurate progress tracking with batching
        progress_bar = tqdm(total=len(ds_ko_full), desc="Processing Korean articles")
        progress_bar.update(len(processed_ko_titles)) # Set initial progress

        for item_ko in ds_ko_full:
            korean_title = item_ko['title']
            if korean_title in processed_ko_titles:     # Check if already processed
                continue
            
            # Add item to the current batch
            batch.append(item_ko)
            ...
            if len(batch) >= BATCH_SIZE:
                ...
                for item_in_batch in batch:
                    ...
                    if result:
                        ...
                        bilingual_record = {
                            "id_ko": item_in_batch['id'], "url_ko": item_in_batch['url'],
                            "title_ko": title_in_batch, "text_ko": item_in_batch['text'],
                            "id_en": item_en['id'],
                        }
                        ...
                        f_ko_only.write(json.dumps(ko_only_record, ensure_ascii=False) + "\n")
        ...
        if batch:
            ...
            for item_in_batch in batch:
                ...
                if result:
                    item_en = json.loads(result[0])
                    bilingual_record = {"id_ko": item_in_batch['id'], "url_ko": item_in_batch['url'], "title_ko": title_in_batch, "text_ko": item_in_batch['text'], "id_en": item_en['id']}
                    f_bilingual.write(json.dumps(bilingual_record, ensure_ascii=False) + "\n")
                else:
                    ko_only_record = {"id_ko": item_in_batch['id'], "url_ko": item_in_batch['url'], "title_ko": title_in_batch, "text_ko": item_in_batch['text']}
                    f_ko_only.write(json.dumps(ko_only_record, ensure_ascii=False) + "\n")
